...
class Infer():
    def __init__(self):
        ...
        imported_graph = tf.train.import_meta_graph('./drowsiness_stable/my_model1.meta')
        self.sess = tf.Session()
        imported_graph.restore(self.sess, './drowsiness_stable/my_model1')
        graph = tf.get_default_graph()


    def how_drowsy(self, blinks):
        labels_net = np.array([10]).reshape([1,1])
        Placeholder = 1
        phase_train = False
        return self.sess.run(['last_fc/mul:0'],
                        feed_dict={
                        'bacth_in:0': blinks,
...
if __name__ == '__main__':
    Blinks = np.load('./drowsiness_stable/Blinks_30_Fold1.npy')
    ...
    bacth_in = Blinks
    infer = Infer()
    print(infer.how_drowsy(bacth_in))
